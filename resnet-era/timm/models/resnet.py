...
class ERA(nn.Module):
    def __init__(self, H_tau_min, n_samples):
        super().__init__()
        self._tau = 4.
        ...
        self.slope = (self.upper_bound - H_tau_min / n_samples) / (1 - 1 / n_samples)
        self.b = (H_tau_min - self.slope) / n_samples
    
    def forward(self, x):
        h = self.slope * x.softmax(dim=-1) + self.b
        u = -1 - torch.log(h)
        ...
        x = x - max_values
        min_values = torch.min(new_logits, dim=-1, keepdim=True).values.detach()
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            H_min (float): Minimum entropy value for ERA
        """
        ...
        self.era = ERA(H_tau_min=math.exp(H_min-1), n_samples=self.num_classes)
